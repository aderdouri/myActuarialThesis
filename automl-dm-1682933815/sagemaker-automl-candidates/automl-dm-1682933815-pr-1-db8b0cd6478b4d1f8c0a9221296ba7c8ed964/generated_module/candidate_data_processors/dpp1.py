...
HEADER = Header(
    column_names=[
        'target', 'PERMIS', 'ACV', 'SEX', 'STATUT', 'CSP', 'USAGE', 'AGECOND',
        'K8000', 'RM', 'CAR', 'CLA', 'ALI', 'ENE', 'VIT', 'SEGM', 'GARAGE'
    ],
...
def build_feature_transform():
    ...
    numeric = HEADER.as_feature_indices(
        ['PERMIS', 'ACV', 'CSP', 'USAGE', 'AGECOND', 'RM', 'GARAGE']
    )
    ...
    categorical = HEADER.as_feature_indices(
        [
            'PERMIS', 'ACV', 'SEX', 'STATUT', 'CSP', 'USAGE', 'AGECOND',
            'K8000', 'RM', 'CAR', 'CLA', 'ALI', 'ENE', 'VIT', 'SEGM', 'GARAGE'
        ]
    )
    ...
    categorical_processors = Pipeline(
        steps=[
            ('thresholdonehotencoder', ThresholdOneHotEncoder(threshold=15))
        ]
    )
    ...
    return Pipeline(
        steps=[
            ('column_transformer',
             column_transformer), ('robustpca', RobustPCA(n_components=78)),
...
def build_label_transform():
    ...
    return RobustLabelEncoder(labels=['0', '1', '2'])
